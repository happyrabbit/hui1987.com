...
- 问题要具体
    ...
    <!--假设领导关心，领导需要。领导不关心的问题短期内是没有未来的，我说的短期可能是大部分人的有生之年。这在中国美国都一样。人性的问题。扯远了。这里假设这是一个政治正确的问题。看过我之前视频的可能马上意识到，这个问题其实很模糊。哪方面的乘客体验？-->
    <!--微信居然能打赏，我真心很兴奋-->
    - 类别：比较、描述、聚类，判别还是回归
...
- 建模和评估：
    ...
    - PCA，CFA
    <!--我们考虑3个常用的可以对数据降维的特征提取方法。主成分分析（PCA）试图找到原变量的不相关线性组合，这些线性组合能够最大限度的解释原数据中变量方差。解释性因子分析（EFA）同样试图在尽量小的维度上解释原数据中尽可能多的方差。高维标度化（MDS）将观测见的相似度映射到低维度上，如2维平面。MDS能够作用于非数值型变量，如分类变量或者有序数据预测变量。接下来我们通过模拟的航空公司数据集来展示不同的特征提取方法。在市场营销中这类消费者调查问卷中，虽然初始问题很多，但通常存在多个调查项共同反应少数几个潜在因子。
    有时间会讲CFA-->
    - 数据划分和再抽样，**评估标准**
